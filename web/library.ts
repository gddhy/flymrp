export type Game = { id: number; name: string; sha256?: string; title?: string; category?: string; size?: number; local?: boolean };
...
export function installedGamesFromSd(files: { path: string; bytes: { length: number } }[]): Game[] {
  const out: Game[] = [];
