...
function frame(now: number): void {
  ...
  try {
    ...
    if (s.rt.unknownRequiredSlot !== null) {
      stop();
      setStatus(`UNKNOWN_REQUIRED_SLOT = ${s.rt.unknownRequiredSlot}`, true);
      return;
    }
...
async function startFromFile(file: File): Promise<void> {
  ...
  const title = archive.header.appname || archive.header.filename || file.name;
  session = { rt, gfx, raf: 0, last: performance.now(), held: new Set() };
  stopBtn.disabled = false;
  canvas.focus();
  setStatus(`${title} 已运行。点画布后用键盘或下方按键操作。`);
