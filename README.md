...
当前：guest inflate 完成；table[30] `mr_getCharBitmap` / table[37] `mr_plat(1206)` / table[26] `mr_printf` **REAL_EXECUTED**。`arm_ext_call(0)` / Lua 未恢复。Stage 5-C **NOT COMPLETE**（blocker=`table[42]` `mr_info`，LIVE `dbglog.txt`）。见 `docs/autonomous-progress.md`。  
