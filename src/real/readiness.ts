...
export function loaderReadiness(): ReadinessItem[] {
  return [
    ...
    { item: "EXT MRPGCMAP", status: "PARTIAL", reason: "cfunction load + code 6 guest return 0; guest inflate completes; DrawRect/DrawText/drawBitmap/winCreate REAL_EXECUTED; production STOP at mr_plat(1205); Stage 5-C NOT COMPLETE" },
    ...
    { item: "150-slot mr_table", status: "PARTIAL", reason: "slots 0/14/25/125/130(case 7)/38(0x4c6+SWITCHPATH)/33/17/40/44/45/41 + 3/10/1/9/30/37/26/42/49/5/35/61/15/6/18/7/122/123/29/78 wired; mr_plat(1205) blocked" },
    { item: "nested EXT owner", status: "PARTIAL", reason: "ModuleOwners in Stage 4; fixtures only" },
    { item: "Lua chunk loading", status: "PARTIAL", reason: "first start.mr; 801/1 and 801/6 return to Lua; 801/0 completes guest inflate then STOP at mr_plat(1205); Lua not resumed" },
    { item: "_runFile / restart", status: "PARTIAL", reason: "state machine exists; synthetic only" },
    { item: "real start.mr / .mrp", status: "PARTIAL", reason: "app.mrp present; deterministic STOP at mr_plat(1205) MR_CHECK_TOUCH; inflate POP return + SHA-256 oracle match; two drawBitmap presents; arm_ext_call(0)/Lua not resumed; Stage 5-C NOT COMPLETE" },
