...
export function loaderReadiness(): ReadinessItem[] {
  return [
    ...
    { item: "150-slot mr_table", status: "PARTIAL", reason: "slots 0/14/25/125/130(case 7)/38(0x4c6)/33/17(%d)/40/44/45/41 current-pack RDONLY + 3 memcpy2 + 10 strcmp2 + 1 mr_free registry-only + 9 memcmp2 + 30 getCharBitmap + 37 plat(1206) + 26 printf wired; gzip/inflate not a host ABI; table[42] mr_info pack/INVALID; table[49] mr_mkDir blocked" },
    { item: "nested EXT owner", status: "PARTIAL", reason: "ModuleOwners in Stage 4; fixtures only" },
    { item: "Lua chunk loading", status: "PARTIAL", reason: "first start.mr; 801/1 and 801/6 return to Lua; 801/0 completes guest inflate then STOP at table[49] mr_mkDir; Lua not resumed" },
    { item: "_runFile / restart", status: "PARTIAL", reason: "state machine exists; synthetic only" },
    { item: "real start.mr / .mrp", status: "PARTIAL", reason: "app.mrp present; deterministic STOP at table[49] mr_mkDir (LIVE gsidbak); inflate POP return + SHA-256 oracle match; arm_ext_call(0)/Lua not resumed; Stage 5-C NOT COMPLETE" },
