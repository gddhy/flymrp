...
export function loaderReadiness(): ReadinessItem[] {
  return [
    ...
    { item: "EXT MRPGCMAP", status: "PARTIAL", reason: "cfunction load + code 6 guest return 0; table[130] case 7 + table[38] 0x4c6 + table[33] getTime + table[17] sprintf_%d + table[40]/[44]/[45] current-pack RDONLY; production STOP at table[3]; 5-C.10L memcpy2 forensics, not implemented" },
    ...
    { item: "Lua chunk loading", status: "PARTIAL", reason: "first start.mr; 801/1 and 801/6 return to Lua; 801/0 hits table[3]; 5-C.10L" },
    { item: "_runFile / restart", status: "PARTIAL", reason: "state machine exists; synthetic only" },
    { item: "real start.mr / .mrp", status: "PARTIAL", reason: "app.mrp present; 5-C.10L deterministic STOP at UNKNOWN_REQUIRED_SLOT=3" },
