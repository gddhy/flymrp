...
export function loaderReadiness(): ReadinessItem[] {
  return [
    ...
    { item: "EXT MRPGCMAP", status: "PARTIAL", reason: "cfunction load + code 6 guest return 0; table[130] case 7 + table[38] 0x4c6 + table[33] getTime + table[17] sprintf_%d + table[40]/[44]/[45]/[41] current-pack RDONLY + table[3] memcpy2 + table[10] strcmp2 + table[1] registry-only mr_free + table[9] memcmp2; guest inflate completes; production STOP at table[30] mr_getCharBitmap; Stage 5-C NOT COMPLETE; 5-C.10R" },
    ...
    { item: "150-slot mr_table", status: "PARTIAL", reason: "slots 0/14/25/125/130(case 7)/38(0x4c6)/33/17(%d)/40/44/45/41 current-pack RDONLY + 3 memcpy2 + 10 strcmp2 + 1 mr_free registry-only + 9 memcmp2 wired; gzip/inflate not a host ABI" },
    { item: "nested EXT owner", status: "PARTIAL", reason: "ModuleOwners in Stage 4; fixtures only" },
    { item: "Lua chunk loading", status: "PARTIAL", reason: "first start.mr; 801/1 and 801/6 return to Lua; 801/0 completes guest inflate then STOP at table[30]; Lua not resumed; 5-C.10R" },
    { item: "_runFile / restart", status: "PARTIAL", reason: "state machine exists; synthetic only" },
    { item: "real start.mr / .mrp", status: "PARTIAL", reason: "app.mrp present; 5-C.10R deterministic STOP at table[30]; inflate POP return + SHA-256 oracle match; arm_ext_call(0)/Lua not resumed; Stage 5-C NOT COMPLETE" },
