...
export function loaderReadiness(): ReadinessItem[] {
  return [
    ...
    { item: "EXT MRPGCMAP", status: "PARTIAL", reason: "cfunction load + code 6 guest return 0; table[130] case 7 + table[38] 0x4c6 + table[33] getTime + table[17] sprintf_%d; production STOP at table[40]; see 5-C.10H" },
    ...
    { item: "150-slot mr_table", status: "PARTIAL", reason: "slots 0/14/25/125/130(case 7)/38(0x4c6)/33/17(%d) wired; 40 not implemented" },
    { item: "nested EXT owner", status: "PARTIAL", reason: "ModuleOwners in Stage 4; fixtures only" },
    { item: "Lua chunk loading", status: "PARTIAL", reason: "first start.mr; 801/1 and 801/6 return to Lua; 801/0 hits table[40]; 5-C.10H" },
    { item: "_runFile / restart", status: "PARTIAL", reason: "state machine exists; synthetic only" },
    { item: "real start.mr / .mrp", status: "PARTIAL", reason: "app.mrp present; 5-C.10H deterministic STOP at UNKNOWN_REQUIRED_SLOT=40" },
