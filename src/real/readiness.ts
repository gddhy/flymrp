...
export function loaderReadiness(): ReadinessItem[] {
  return [
    ...
    { item: "EXT MRPGCMAP", status: "PARTIAL", reason: "cfunction load + code 6 guest return 0; table[130] case 7 + table[38] 0x4c6 + table[33] getTime + table[17] sprintf_%d + table[40]/[44]/[45]/[41] current-pack RDONLY + table[3] memcpy2 + table[10] strcmp2 + table[1] registry-only mr_free + table[9] memcmp2; guest inflate completes; production STOP at table[30] mr_getCharBitmap; 5-C.10Q" },
    ...
    { item: "Lua chunk loading", status: "PARTIAL", reason: "first start.mr; 801/1 and 801/6 return to Lua; 801/0 completes guest inflate then STOP at table[30]; 5-C.10Q" },
    { item: "_runFile / restart", status: "PARTIAL", reason: "state machine exists; synthetic only" },
    { item: "real start.mr / .mrp", status: "PARTIAL", reason: "app.mrp present; 5-C.10Q deterministic STOP at table[30]; guest inflate output matches reference gunzip; _mr_readFile(res_lang0.rc) succeeded" },
