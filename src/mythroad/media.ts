...
type Media = { device: number; data: Uint8Array | null; loop: number; status: number };
...
export class MediaDevices {
  ...
  constructor(private hooks: {
    ...
    play: (type: number, bytes: Uint8Array, loop: number) => void;
  ...
  dispatch(mem: GuestMemory, code: number, input: number, length: number, output: number, outputLength: number): number | null {
    ...
    if (!device || ![201,202,203,204,205,206,207,208,209,216,222,223,224,225].includes(command)) return null;
    if (device > 7) return 1;
    ...
    switch (command) {
      case 201: case 208: case 216:
        this.hooks.stop(type); media.data = null; media.status = command === 201 ? 2 : 1; return 0;
      case 202: case 203: {
        ...
        this.hooks.stop(type); media.data = data.slice(); media.status = 3; return 0;
      }
      case 204: case 206:
        if (!media.data) return -1;
        if (input && length >= 12) media.loop = mem.read32(input + 4) | 0;
        this.hooks.play(type, media.data, media.loop); media.status = 4; return 0;
      case 205: case 207:
        this.hooks.stop(type); media.status = command === 205 ? 5 : 3; return 0;
      case 209:
        if (output && outputLength) {
          this.statusAddress ||= this.hooks.alloc(4);
          mem.write32(this.statusAddress, media.status);
