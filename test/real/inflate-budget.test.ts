...
describe("5-C.10Q forensic ARM insn watchdog", () => {
  ...
  it("sufficient budget reaches table[32]; 2M and 5M are the same guest result", () => {
    const bytes = new Uint8Array(readFileSync(REAL_APP));
    const a = runInflateBudget(bytes, { budget: 2_000_000 });
    const b = runInflateBudget(bytes, { budget: 5_000_000 });
    expect(a.thrown).toBe(UNKNOWN_SLOT_32_THROWN);
    expect(b.thrown).toBe(UNKNOWN_SLOT_32_THROWN);
    expect(a.unknownSlot).toBe(32);
    expect(b.unknownSlot).toBe(32);
