...
describe("5-C.5 real loader chain", () => {
  it("cfunction load + code 6 return, then stops at mr_plat(1205)", () => {
    ...
    rt.loadMrp(bytes);
    expect(() => rt.start("start.mr")).toThrow(/unsupported mr_plat code 1205/);
    expect(rt.unknownRequiredSlot).toBeNull();
    expect(rt.unknownEvents.some((e) => e.family === "mr_plat" && e.code === 1205)).toBe(true);
