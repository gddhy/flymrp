...
describe("5-C.5 real loader chain", () => {
  it("cfunction load + code 6 return, then stops at table[130]", () => {
    ...
    expect(rt.unknownRequiredSlot).toBe(130);
    expect(rt.unknownEvents.some((e) => e.family === "mr_table" && e.code === 130)).toBe(true);
    ...
    expect(rt.ext?.owners.wrapper.p).toBe(0x00200100);
