...
describe("5-C.10R post-inflate startup gate", () => {
  ...
  it("production watchdog completes guest inflate and stops at table[122] DrawRect", () => {
    const bytes = new Uint8Array(readFileSync(REAL_APP));
    ...
    expect(r.thrown).toBe(UNKNOWN_SLOT_122_THROWN);
    expect(r.unknownSlot).toBe(122);
    ...
    expect(new Set(r.consistency.fingerprints.map((f) => f.thrown))).toEqual(new Set([UNKNOWN_SLOT_122_THROWN]));
  });
});
