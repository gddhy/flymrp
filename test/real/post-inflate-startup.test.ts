...
describe("5-C.10R post-inflate startup gate", () => {
  ...
  it("production watchdog completes guest inflate and stops at table[30]", () => {
    const bytes = new Uint8Array(readFileSync(REAL_APP));
    const r = runPostInflateStartup(bytes, { consistencyRuns: 5 });

    expect(r.watchdog).toBe(DEFAULT_INSN_BUDGET);
    expect(r.insnCount).toBe(REAL_MRP_BASELINE.inflateInsnCount);
    expect(r.insnCount).toBeGreaterThan(1_000_000);
    expect(r.tableStubCount).toBe(POST_INFLATE.hitCount);
    expect(r.thrown).toBe(UNKNOWN_SLOT_30_THROWN);
    expect(r.unknownSlot).toBe(30);
    ...
    expect(r.graphicsCommands).toBe(0);
    ...
    expect(new Set(r.consistency.fingerprints.map((f) => f.thrown))).toEqual(new Set([UNKNOWN_SLOT_30_THROWN]));
  });
});
