...
import { REAL_MRP_BASELINE, UNKNOWN_SLOT_49_THROWN, runRealMrpStartup } from "../../src/real/startup.ts";
...
describe("5-C.10Q real MRP startup after guest inflate", () => {
  it("REAL_EXECUTED table9 memcmp2; guest inflate completes; stops at table[49]", () => {
    ...
    expect(r.lua.exception?.message).toBe(UNKNOWN_SLOT_49_THROWN);
    ...
    expect(r.mrTable.hits.at(-1)!.slot).toBe(49);
    ...
    expect(bySlot[49]!.status).toBe("NOT_EXECUTED");
    expect(bySlot[49]!.guestReached).toBe(true);
    expect(bySlot[49]!.handlerPresent).toBe(false);
    ...
    expect(r.stop.reason).toBe(UNKNOWN_SLOT_49_THROWN);
    expect(r.stop.slot).toBe(49);
    ...
    expect(r.baseline.firstProductionBlocker).toBe("table[49]");
    expect(r.baseline.firstPost130Blocker).toBe("table[49]");
    ...
    expect(fp.firstUnknownSlot).toBe(49);
  ...
  it("advance(N) before start stores N at ER_RW+0x4358 without changing baseline order", () => {
    ...
    try {
      ...
      expect(e).toMatchObject({ message: UNKNOWN_SLOT_49_THROWN });
    }
    expect(rt.unknownRequiredSlot).toBe(49);
