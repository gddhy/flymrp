...
import { REAL_MRP_BASELINE, UNKNOWN_PLAT_1205_THROWN, runRealMrpStartup } from "../../src/real/startup.ts";
...
describe("5-C.10Q real MRP startup after guest inflate", () => {
  it("REAL_EXECUTED table9 memcmp2; guest inflate completes; stops at mr_plat(1205)", () => {
    ...
    expect(r.lua.exception?.message).toBe(UNKNOWN_PLAT_1205_THROWN);
    ...
    expect(r.mrTable.hits.at(-1)!.slot).toBe(37);
    ...
    expect(r.stop.reason).toBe(UNKNOWN_PLAT_1205_THROWN);
    ...
    expect(r.baseline.firstProductionBlocker).toBe(UNKNOWN_PLAT_1205_THROWN);
    expect(r.baseline.firstPost130Blocker).toBe(UNKNOWN_PLAT_1205_THROWN);
  ...
  it("advance(N) before start stores N at ER_RW+0x4358 without changing baseline order", () => {
    ...
    try {
      ...
      expect(e).toMatchObject({ message: UNKNOWN_PLAT_1205_THROWN });
    }
