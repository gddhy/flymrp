...
describe("5-C.5 cfunction init / code-6 forensics", () => {
  ...
  it("memset lets code 6 enter guest; first remaining fault is EFS mr_open", () => {
    ...
    expect(r.fault.subtype).toBe(UNKNOWN_OPEN_GSSJXZ69_THROWN);
    expect(r.fault.site).toBe("arm_ext_call.table");
    expect(r.fault.slot).toBe(40);
    expect(r.fault.kind).toBe("UNKNOWN_OPEN");
    expect(r.fault.faultPc).toBe(0x000100a0);
