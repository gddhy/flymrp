...
describe("5-C.5 cfunction init / code-6 forensics", () => {
  ...
  it("memset lets code 6 enter guest; first remaining fault is table[9] memcmp2", () => {
    ...
    expect(r.fault.subtype).toBe("UNKNOWN_REQUIRED_SLOT = 9");
    expect(r.fault.site).toBe("arm_ext_call.table");
    expect(r.fault.slot).toBe(9);
    expect(r.fault.faultPc).toBe(0x10024);
    expect(r.fault.r0).toBe(0x01e7fee8);
    expect(r.fault.r1).toBe(0x01eadefc);
    expect(r.fault.r2).toBe(2);
