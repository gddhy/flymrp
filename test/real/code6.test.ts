...
describe("5-C.5 cfunction init / code-6 forensics", () => {
  ...
  it("memset lets code 6 enter guest; first remaining fault is mr_plat(1205)", () => {
    ...
    expect(r.fault.subtype).toBe("unsupported mr_plat code 1205");
    expect(r.fault.site).toBe("arm_ext_call.table");
    expect(r.fault.slot).toBe(37);
    expect(r.fault.kind).toBe("UNKNOWN_PLAT");
    expect(r.fault.faultPc).toBe(0x00010094);
