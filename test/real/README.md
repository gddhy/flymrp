...
`loader-abi.test.ts` is the real-app chain: `start.mr` → `mrc_loader.ext` → `cfunction.ext` load → `arm_ext_call(6)` guest return 0, then stop at table[35] after guest inflate.
...
`real-mrp-startup.test.ts` 是真实 `app.mrp` 生产启动。guest inflate 完成，table[30]/[37]/[26]/[42]/[49]/[5] REAL_EXECUTED，停在 table[35] `mr_getUserInfo`。No forensic bypass。
...
`file-chain-forensics.test.ts` is Stage 5-C.10J：`_mr_readFile` pack-file ABI 静态链与只读 handle 设计。5-C.10K 已实现 40/44/45/41。当前生产停在 table[35]。

`memcpy-3-forensics.test.ts` is Stage 5-C.10L：table[3] `memcpy2` LIVE 首笔 ABI 与 directory loop 后续 slot。5-C.10M 已实现 3/10；该取证仍锁第一笔 table[3]，当前生产停在 table[35]。
