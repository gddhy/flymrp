...
**real-app green: false。** Stage 5-C.5：`table[14]` memset 已接；cfunction load 完成；`arm_ext_call(6)` guest 返回 0。完整 runtime 停在 `table[130]`（`UNKNOWN_REQUIRED_SLOT = 130`）。未实现 TestCom / host code 6 helper。见 `docs/stage5c5-progress.md`。
