...
fail（`UNKNOWN_REQUIRED_SLOT = 49`）。guest inflate POP 返回，输出 SHA-256 与 reference gunzip 一致。table[30]/[37]/[26]/[42] **REAL_EXECUTED**。`arm_ext_call(0)` / Lua 未恢复。Stage 5-C **NOT COMPLETE**。见 `docs/autonomous-progress.md`。

## Lua execution

fail — `_strCom(801,"",0)` / `arm_ext_call(0)` 完成 guest inflate 后停在 table[49] `mr_mkDir("gsidbak")`

## Native ABI

- confirmed calls: `_com(3629,2913)`，`GetSysInfo`，`_strCom(601/800/801)`，`string.unpack("II")`
- unknown calls: **none**（Lua）
- unknown required slot: **table[49] `mr_mkDir`**。LIVE name `gsidbak`。
...
- `arm_ext_call(0)`：table[130] case 7 **REAL_EXECUTED**（r0=`0x270f`，ER_RW+0x1c=`0x270d`）→ table[14] → table[38] code 0x4c6 **REAL_EXECUTED** → table[33] `mr_getTime` **REAL_EXECUTED** → table[17] `sprintf_` **REAL_EXECUTED** → table[40]/[44]/[45]/[41]/[43] **REAL_EXECUTED**（current-pack RDONLY + AppFS EFS）→ table[3] memcpy2 **REAL_EXECUTED** → table[10] strcmp2 **REAL_EXECUTED**（命中 `res_lang0.rc`，file_pos=7065 file_len=17174 与 `MRPArchive` 一致）→ table[1] **REAL_EXECUTED** → table[9] memcmp2 **REAL_EXECUTED** → guest inflate **PASS** → DrawRect/DrawText/drawBitmap/winCreate/plat(1205) **REAL_EXECUTED** → AppFS `gssjxz\\69` **REAL_EXECUTED** → table[32]/[31] timer **REAL_EXECUTED** → table[80] getScreenInfo **REAL_EXECUTED** → **arm_ext_call(0) NORMAL RETURN**；Lua resumes
