...
当前增量：Stage 5-C.10O（`docs/stage5c10o-progress.md`）实现 table[1] registry-only `mr_free`。生产停在 table[9] `memcmp2`。guest `_mr_readFile("res_lang0.rc")` 已完整成功。
...
| A | 29 | 冻结（5-B） |
| B | 26 | 冻结（5-B） |
| C | 35 | 上表 + `mr_table/0`、`mr_table/14`、`mr_table/125`、`mr_table/130/7`、`mr_table/38/0x4c6`、`mr_table/33/mr_getTime`、`mr_table/17/sprintf/%d`、`mr_table/100/pack_filename`、`mr_table/40/current-pack/RDONLY`、`mr_table/44/read`、`mr_table/45/seek`、`mr_table/41/close`、`mr_table/3/memcpy2`、`mr_table/10/strcmp2`、`mr_table/1/mr_free/registry-only` |
...
| real MRP startup | `test/real/real-mrp-startup.test.ts` | 3 | 真实 app.mrp；table[1]/[41] REAL_EXECUTED；停 9 |
...
`npm test`：**508/508**。`tsc --noEmit` 通过。
