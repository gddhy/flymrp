...
状态：核心文件/inflate 路径已接通。Stage 5-C **NOT COMPLETE**（`table[30]` EXT_ABI）。**不要进入 Stage 5-D。**
...
当前增量：Stage 5-C.10R（`docs/stage5c10r-progress.md`）生产路径 post-inflate continuation / completion gate。guest inflate **PASS**；`arm_ext_call(0)` / Lua **未**恢复。Stage 5-C **NOT COMPLETE**（`table[30]` EXT_ABI）。**未实现** table[30] / gzip host ABI。**不要进入 Stage 5-D。**
