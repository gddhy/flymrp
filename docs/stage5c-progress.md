...
当前增量：Stage 5-C.10M（`docs/stage5c10m-progress.md`）实现 table[3] memcpy2 + table[10] strcmp2。生产停在 table[1] mr_free。未实现 free。
...
`npm test`：**499/499**。`tsc --noEmit` 通过。
