...
Stage 5-C.10O：实现 `table[1]` registry-only `mr_free`。未实现 table[9]。生产停在 table[9]。见 `docs/stage5c10o-progress.md`。  
...
| 路径 | 作用 |
|---|---|
| `src/real/startup.ts` | 5-C.10O 真实 MRP 生产启动基线（table[1]/[41] REAL_EXECUTED；`_mr_readFile` 成功；停在 table[9]） |
