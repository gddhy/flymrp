...
Stage 5-C.10J：current-pack file ABI 只读取证。未实现 table[40]/41+。见 `docs/stage5c10j-progress.md`。  
...
| 路径 | 作用 |
|---|---|
| `src/real/startup.ts` | 5-C.10I 真实 MRP 生产启动基线（table[100] pack_filename；不实现 table[40]） |
